...
cmd:text("")
cmd:option('-data_file', 'data/demo-train.hdf5',
           [[Path to the training *.hdf5 file from preprocess.py.]])
cmd:option('-val_data_file', 'data/demo-val.hdf5',
           [[Path to validation *.hdf5 file from preprocess.py.]])
cmd:option('-savefile', 'seq2seq_lstm_attn',
           [[Savefile name (model will be saved as
             savefile_epochX_PPL.t7 where X is the X-th epoch and PPL is
             the validation perplexity.]])
cmd:option('-num_shards', 0,
           [[If the training data has been broken up into different shards,
             then training files are in this many partitions.]])
cmd:option('-train_from', '',
...
cmd:option('-res_net', 0, [[Use residual connections between LSTM stacks whereby the input to
                          ...
                          experiments.]])
cmd:option('-guided_alignment', 0,
           [[If 1, use external alignments to guide the attention weights as in
...
cmd:option('-guided_alignment_decay', 1,
           [[decay rate per epoch for alignment weight - typical with 0.9,
...
cmd:option('-learning_rate', 1,
           [[Starting learning rate. If adagrad/adadelta/adam is used,
...
function train(train_data, valid_data)
    ...
    --subtract shared params for brnn
    if opt.brnn == 1 then
    ...
    -- offset depends on input feeding
    dec_offset = 3
    if opt.input_feed == 1 then
    ...
    function train_batch(data, epoch)
        ...
        for i = 1, data:size() do
            ...
            if opt.guided_alignment == 1 then
                ...
                if opt.gpuid >= 0 then
                    cutorch.setDevice(opt.gpuid)
                    -- alignment is in the 2nd GPU
                    if opt.gpuid2 >= 0 then
                        cutorch.setDevice(opt.gpuid2)
                    end
                    replicator = replicator:cuda()
                end
                norm_alignment = torch.cdiv(alignment,
                                            replicator:forward(torch.sum(alignment, 2):squeeze(2)))
                norm_alignment[norm_alignment:ne(norm_alignment)] = 0
            end

            local encoder_grads = encoder_grad_proto[{{1, batch_l},
                                                     o{1, source_l}}]
            local encoder_bwd_grads
            if opt.brnn == 1 then
                encoder_bwd_grads = encoder_bwd_grad_proto[{{1, batch_l},
                                                            {1, source_l}}]
            end
            if opt.gpuid >= 0 then
                cutorch.setDevice(opt.gpuid)
            ...
            for t = 1, target_l do
                ...
                local decoder_input
                if opt.attn == 1 then
                    decoder_input = {target[t], context,
                                     table.unpack(rnn_state_dec[t - 1])}
                else
                    decoder_input = {target[t], context[{{}, source_l}],
                                     table.unpack(rnn_state_dec[t - 1])}
                end
    ...
    for epoch = opt.start_epoch, opt.epochs do
        ...
        local savefile = string.format('%s_epoch%.2f_%.2f.t7', opt.savefile,
                                       epoch, score)
...
function eval(data)
    ...
    for i = 1, data:size() do
        ...
        if opt.gpuid >= 0 and opt.gpuid2 >= 0 then
            cutorch.setDevice(opt.gpuid2)
            local context2 = context_proto2[{{1, batch_l},
                                             {1, source_l}}]
            context2:copy(context)
            context = context2
        end
...
function main()
    ...
    print(string.format('Number of additional features on source side: %d',
                        valid_data.num_source_features))
    ...
    if opt.optim ~= 'sgd' then
        ...
        optStates = {}
        for i = 1, #layers do
            -- can have layer-specific lr, if desired
