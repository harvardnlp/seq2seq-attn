...
function generate_aligns(batch_sent_idx, alignment_cc_colidx, alignment_cc_val,
                         source_l, target_l, opt_start_symbol)
    ...
    for k = 1, batch_size do
        ...
        for i = 0, source_l-1 do
            t[k][i+1]:copy(alignment_cc_val:narrow(1,
                                                   alignment_cc_colidx[sent_idx+1+i+src_offset]+1, target_l))
        end
    end
...
function data:__init(opt, data_file)
    ...
    --max source length each batch
    self.source_l = f:read('batch_w'):all()
    ...
    if opt.start_symbol == 0 then
        ...
        for i = 1, self.num_source_features do
            self.source_features[i] = self.source_features[i][
                {{}, {2, self.source_features[i]:size(2)-1}}]
        end
    end
    ...
    for i = 1, self.length do
        ...
        local target_output_i = self.target_output:sub(self.batch_idx[i],
                                                       self.batch_idx[i] + self.batch_l[i]-1, 1, self.target_l[i])
        ...
        if opt.use_chars_dec == 1 then
            ...
            target_i = self.target:sub(self.batch_idx[i],
                                       self.batch_idx[i] + self.batch_l[i]-1,
        ...
        for j = 1, self.num_source_features do
            ...
            if opt.reverse_src == 1 then
                source_feats[j] = source_feats[j]:index(1,
                                                        source_l_rev[{{max_source_l - self.source_l[i] + 1,
                                                                       max_source_l}}])
            end
        end
        ...
        local alignment_i
        if opt.guided_alignment == 1 then
            alignment_i = self.alignment_cc_sentidx:sub(self.batch_idx[i],
                                                        self.batch_idx[i] + self.batch_l[i]-1)
        end
...
function data.__index(self, idx)
    if type(idx) == "string" then
        ...
        local alignment = generate_aligns(self.batches[idx][10],
                                          ...
                                          target_l,
                                          opt.start_symbol)
        -- If multi-gpu, source lives in gpuid1, rest on gpuid2.
        if opt.gpuid >= 0 then
