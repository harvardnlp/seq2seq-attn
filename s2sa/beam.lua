...
cmd:option('-targ_file', '',
           'True target sequence (optional)')
cmd:option('-output_file', 'pred.txt',
           [[Path to output the predictions
           (each line will be the decoded sequence).]])
cmd:option('-src_dict', 'data/demo.src.dict',
           'Path to source vocabulary (*.src.dict file)')
cmd:option('-targ_dict', 'data/demo.targ.dict',
           'Path to target vocabulary (*.targ.dict file)')
cmd:option('-feature_dict_prefix', 'data/demo',
           [[Prefix of the path to features vocabularies
            (*.feature_N.dict files).]])
cmd:option('-char_dict', 'data/demo.char.dict',
           [[If using chars, path to character vocabulary
            (*.char.dict file).]])

-- beam search options
cmd:option('-beam', 5, 'Beam size')
cmd:option('-max_sent_l', 250,
           [[Maximum sentence length.
            If any sequences in srcfile are longer
            than this then it will error out.]])
cmd:option('-simple', 0,
           [[If = 1, output prediction is simply the first time the top of the beam
            ends with an end-of-sentence token. If = 0, the model considers all
            hypotheses that have been generated that ends with end-of-sentence
...
cmd:option('-replace_unk', 0,
           [[Replace the generated UNK tokens with the source token that
             ...
             it will lookup the identified source token and give corresponding
             target token. If it is not provided (or identified source token
             does not exist in the table) then will copy the source token.]])
cmd:option('-srctarg_dict', 'data/en-de.dict',
           [[Path to source-target dictionary to replace UNK
             tokens. See README.md for the format this file should be in.]])
cmd:option('-score_gold', 0,
           'If = 1, score the log likelihood of the gold as well')
cmd:option('-n_best', 1,
...
cmd:option('-gpuid2', -1,
           'Second GPU ID')
cmd:option('-cudnn', 0,
           [[If using character model, this should be = 1
            if the character model was trained using cudnn.]])

cmd:option('-rescore', '',
           [[Use specified metric to select best translation
            in the beam, available: bleu, gleu.]])
cmd:option('-rescore_param', 4,
           [[Parameter for the scoring metric, for BLEU
            is corresponding to n_gram.]])
...
function generate_beam(model, initial, K, max_sent_l, source,
    ...
    while (not done) and (i < n) do
        ...
        if model_opt.use_chars_dec == 1 then
            local narrow = next_ys:narrow(1, i-1, 1):squeeze()
            decoder_input1 = word2charidx_targ:index(1, narrow)
        else
        ...
        if model_opt.attn == 1 then
            decoder_input = {decoder_input1, context,
                             table.unpack(rnn_state_dec)}
        else
            decoder_input = {decoder_input1, context[{{}, source_l}],
                             table.unpack(rnn_state_dec)}
        end
        ...
        if i == 2 then
            -- All outputs same for first batch
            flat_out = out_float[1]
        end
            ...
            while true do
                ...
                if i < 2 or diff then
                    if model_opt.attn == 1 then
                        ...
                        attn_argmax[i][k] = State.advance(attn_argmax[i-1][prev_k],
                                                          max_index[1])
                    end
                    prev_ks[i][k] = prev_k
                    next_ys[i][k] = y_i
    ...
    if opt.rescore ~= '' then
        ...
        for k = 1, K do
                ...
                local score_k = scorers[opt.rescore](hyp, gold_table,
                                                     opt.rescore_param)
    ...
    if opt.score_gold == 1 then
        ...
        for t = 2, target_l do
            ...
            local decoder_input
            if model_opt.attn == 1 then
                decoder_input = {decoder_input1, context[{{1}}],
                                 table.unpack(rnn_state_dec)}
            else
    ...
    if opt.rescore ~= '' then
        local max_mscore = scorers[opt.rescore](max_hyp, gold_table,
                                                opt.rescore_param)
        print('RESCORE MAX ' .. opt.rescore .. ': ' .. max_mscore,
              'BEST ' .. opt.rescore .. ': ' .. best_mscore)
        max_hyp = mscore_hyp
...
function init(arg)
    ...
    for i = 1, model_opt.num_source_features do
        table.insert(idx2feature_src,
                     idx2key(opt.feature_dict_prefix ..
                             '.source_feature_' .. i .. '.dict'))
    ...
    if model_opt.use_chars_dec == 1 then
        word2charidx_targ = torch.LongTensor(#idx2word_targ,
                                             model_opt.max_word_l):fill(PAD)
        for i = 1, #idx2word_targ do
            word2charidx_targ[i] = word2charidx(idx2word_targ[i],
                                                char2idx,
                                                model_opt.max_word_l,
                                                word2charidx_targ[i])
        end
    end
    ...
    if opt.gpuid >= 0 then
        ...
        if opt.gpuid2 >= 0 then
            ...
            cutorch.setDevice(opt.gpuid2)
            context_proto2 = torch.zeros(opt.beam, MAX_SENT_L,
                                         model_opt.rnn_size):cuda()
        else
            context_proto = context_proto:cuda()
        end
        if model_opt.attn == 1 then
...
function search(line)
    ...
    local source_features = features2featureidx(source_features_str,
                                                feature2idx_src,
                                                model_opt.start_symbol)
    if model_opt.use_chars_enc == 0 then
        source, source_str = sent2wordidx(cleaned_line, word2idx_src,
                                          model_opt.start_symbol)
    else
        source, source_str = sent2charidx(cleaned_line, char2idx,
                                          model_opt.max_word_l,
                                          model_opt.start_symbol)
    end
    if gold then
    ...
    pred, pred_score, attn, gold_score, all_sents, all_scores, all_attn = generate_beam(model,
                                                                                        state, opt.beam, MAX_SENT_L,
                                                                                        source, source_features, target)
    ...
    if gold ~= nil then
        ...
        if opt.score_gold == 1 then
            print(string.format("PRED SCORE: %.4f, GOLD SCORE: %.4f",
    ...
    if opt.n_best > 1 then
        for n = 1, opt.n_best do
            pred_sent_n = wordidx2sent(all_sents[n], idx2word_targ, source_str,
                                       all_attn[n], false)
            local out_n = string.format("%d ||| %s ||| %.4f", n,
                                        pred_sent_n, all_scores[n])
            print(out_n)
            nbests[n] = out_n
        end
    end
